<template>
  <div class="bannerEdit">
        ...
        <div class="picItem" :index="i" v-for="(v,i) in saveContent" :key="i">
          ...
          <div class="addPic">
            <div :class="['add',{'alert':v.imgRequired}]" @mouseover="mouseover(i)" @mouseout="mouseout(i)">
              <img :class="'showPic'+i" :src="v.srcImg" ref="uploadPic" v-show="v.srcImg">
            ...
            <div class="picLink">
              <span class="linkContent">
                <el-input class="link text" type="text" clearable placeholder="请输入合法的无线连接" v-model="v.url"></el-input>
    ...
    <search-pop :index="index" v-if="isShowPop" @closePop="closePop" @confirmSelect="confirmSelect"></search-pop>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      picSrc: [],
      defaultImg:
        "https://gw.alicdn.com/tfs/TB1nkM1SXXXXXXmaXXXXXXXXXXX-750-359.gif",
      isShowLoading: false, //加载动画
      nowUpload: 0, //当前进行图片上传的位置
      isShowPop: false,
      selectGoodItem: {}, //选择的商品信息
  ...
  computed: {
    ...
    saveContent() {
      this.picSrc = this.dragContent[this.index].dataJson.swiperImg;
  ...
  methods: {
    ...
    picUpload() {
      ...
      let type = fileData.type.split("/")[1].toLowerCase();
      ...
      this.uploadPic(fileData, data => {
        let index = this.nowUpload;
        this.picSrc[index].srcImg = data;
    ...
    confirmSelect(goodLink) {
      this.picSrc[this.popIndex].url = goodLink;
      this.closePop();
    },
    //增加图片容器
    addContent() {
