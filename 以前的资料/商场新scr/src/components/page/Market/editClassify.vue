    ...
    <div class="formdiv">
      <el-form ref="ruleForm" label-width="120px" class="demo-ruleForm">
        <el-form-item label="分类名称:" prop="name">
          <el-input v-model="ruleForm.industryName" style="width:400px;" size="medium" maxlength="20"></el-input>
          <span class="tiptext">{{ruleForm.industryName.length}}/20</span>
        </el-form-item>
        <el-form-item label="分类排序:">
          <el-input type="number" v-model="ruleForm.sort" style="width:400px;" size="medium" maxlength="20"></el-input>
        </el-form-item>
        <el-form-item label="分类图标:" v-show="parentId==0">
...
export default {
  ...
  methods: {
      ...
      this.$httpAuth({
        ...
        .then(res => {
          if (res.data.code == 10000) {
            ...
            _this.ruleForm = resp;
            if (this.parentId == 0) {
              _this.dialogImageUrl.push(resp.icon);
            }
          }
        })
        .catch(err => {
    ...
    onSubmit() {
      let _this = this;
      if (_this.ruleForm.industryName == "") {
        this.$message.info("分类名称不能为空");
        return;
      } else if (specailTest(this.ruleForm.industryName)) {
      ...
      this.$httpAuth({
        ...
        data: {
          ...
          industryName: _this.ruleForm.industryName,
          sort: _this.ruleForm.sort,
          icon: _this.dialogImageUrl[0]
        }
      })
        .then(res => {
          if (res.data.code == 10000) {
